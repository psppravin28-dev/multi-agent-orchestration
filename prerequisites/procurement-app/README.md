...
for the mechanism (`join_transact# Procurement app
...
**`python -m app.seed` is not idempotent — running it twice duplicates every
vendor and contract.** The script has no check for existing data; it just
inserts. If you seeded more than once, `GET /vendors` will return doubled
(or tripled) entries with different `id`s but identical names. Reset:
```bash
docker exec procurement-postgres psql -U procurement -d procurement -c "TRUNCATE vendor, contract, purchaseorder RESTART IDENTITY CASCADE;"
python -m app.seed
```
This is on the roadmap to fix properly (make `seed()` check for existing
rows before inserting) — for now, just don't run `python -m app.seed` more
than once per fresh database.
...
- Make `seed()` idempotent (check for existing rows before inserting) —
  see the note under "Running it"; currently a manual TRUNCATE is the fix
...
ion_mode="create_savepoint"`) — verified
by running the suite three times in a row against the same database with
zero leakage before this was written into the fixture.
