...
Read [[Home]] and [[Module 1 Hierarchical Delegation]] first — this
module is explained by contrast with Module 1's router-function
approach. Have the repo's
[02-handoff-patterns/](https://github.com/psppravin28-dev/multi-agent-orchestration/tree/main/02-handoff-patterns)
folder open alongside this.
...
Open [exercise1_command_handoff.py](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise1_command_handoff.py).
...
1. [`intake_agent`](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise1_command_handoff.py#L59-L75) —
   classifies the request via structured output, same as before, **but
   instead of returning a dict**, it returns
   `Command(goto=decision.next_agent, update={...})`. The routing
   decision (`goto`) and the state update happen in one object, from
   inside the node itself.
2. [`vendor_sourcing`](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise1_command_handoff.py#L78-L84) and
   [`compliance`](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise1_command_handoff.py#L87-L93) —
   each does its API call, then returns
   `Command(goto=END, update={"result": ...})`. Notice: **`END` is a
   valid `goto` target**, not just a graph-level edge.
3. [The wiring](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise1_command_handoff.py#L100-L105) —
   read the comment on lines 96-99 carefully. There's
   `add_edge(START, "intake_agent")` and *nothing else*. No
   `add_conditional_edges`, no edges out of `intake_agent` at all. Each
   node's own `Command.goto` **is** the edge — LangGraph doesn't need
   edges declared in advance for a `Command`-returning node.

Compare this directly against Module 1 Exercise 2's
[`route_after_supervisor`](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/01-hierarchical-delegation/exercise2_supervisor_routing.py#L89-L90) +
`add_conditional_edges` — same routing outcome, achieved by collapsing
"decide" and "act" into one function instead of two.
...
Open [exercise2_peer_handoff_chain.py](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise2_peer_handoff_chain.py).
...
1. [`intake_agent`](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise2_peer_handoff_chain.py#L64-L81) —
   extracts contract details, hands off to `contract_review`. Nothing
   new yet.
2. [`contract_review`](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise2_peer_handoff_chain.py#L84-L112) —
   this is the important one. It creates the contract via the real API,
   **then decides for itself** (lines 98-101) whether the deal is big
   enough to also need a compliance check:
   ```python
...
3. [`compliance`](https://github.com/psppravin28-dev/multi-agent-orchestration/blob/main/02-handoff-patterns/exercise2_peer_handoff_chain.py#L115-L131) —
   same shape as before, finishes at `END`.
